...
REGISTRY="${REGISTRY:-'cnfde2.ptp.lab.eng.bos.redhat.com:5000'}"
MIRROR="${MIRROR:-'cnfde2.ptp.lab.eng.bos.redhat.com:6666'}"
...
podman build -f Dockerfile --no-cache . \
  ...
  -t ${REGISTRY}/${USER}/${DRIVER_IMAGE}:${TAG}

podman push --tls-verify=false ${REGISTRY}/${USER}/${DRIVER_IMAGE}:${TAG}
